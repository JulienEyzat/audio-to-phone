...
class MTP:
	...
	def send_file_from_file(self, source, target, parent_id, callback=None):
		...
		true_target = target.encode("utf-8")
		true_source = source.encode("utf-8")

		metadata = LIBMTP_File(filename=true_target, filetype=self.find_filetype(source), filesize=os.stat(source).st_size, parent_id=int(parent_id))

		ret = self.mtp.LIBMTP_Send_File_From_File(self.device, true_source, ctypes.pointer(metadata), callback, None)
	...
	def explore_children(self, directory_id):
		...
		next = self.mtp.LIBMTP_Find_Folder(folders, int(directory_id))
		ret = {}
		while True:
			next = next.contents
			scanned = True

			if not (next.folder_id in ret):
			...
			elif (next.parent_id != directory_id):
		...
		ret.pop(directory_id)
		return ret

	def get_folder_list_in_folder(self, directory_name, parent_directory_id):
		...
		to_explore_folder = None
		while True:
			next = next.contents
			scanned = True
			folder_name = next.name.decode("utf-8")
			parent_id = int(next.parent_id)

			# Check if this ID exists, if not, add it
			# and trigger a scan of the children
			if parent_id == parent_directory_id and folder_name == directory_name:
				to_explore_folder = next.folder_id
				break

			if not (next.folder_id in scanned_folders):
				scanned_folders.append(next.folder_id)
				scanned = False
		...
		if to_explore_folder is not None:
			ret = self.explore_children(to_explore_folder)
		else:
			ret = None
		return ret
	...
	def get_folder_id(self, name, parent_directory_id):
		if (self.device == None):
			raise NotConnected
		...
		while True:
			...
			# Check if this ID exists, if not, add it
			# and trigger a scan of the children
			if parent_id == parent_directory_id and folder_name == name:
	...
	def create_folder_by_id(self, name, parent=0, storage=0):
		...
		ret = self.mtp.LIBMTP_Create_Folder(self.device, binary_name, parent, storage)
